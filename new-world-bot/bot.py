...
class NewWorldBot:
    ...
    def __init__(self, window_offset, window_size):
        ...
        self.harvest_tooltip = cv.imread("harvest_tooltip.jpg", cv.IMREAD_UNCHANGED)
    ...
    # This is probably not needed? We will need moveToNextTarget instead?
    # def click_next_target(self):
    #     # 1. order targets by distance from center
    #     # loop:
    #     #   2. hover over the nearest target
    #     #   3. confirm that it's harvestable via the tooltip
    ...
    #     target_i = 0
    #     found_harvestable = False
    #     while not found_harvestable and target_i < len(targets):
    ...
    def targets_ordered_by_distance(self, targets):
        ...
        # TODO that is wrong in our case i think.. lets discuss this
        targets = [t for t in targets if pythagorean_distance(t) > self.IGNORE_RADIUS]

        return targets

    def confirm_tooltip(self, target_position):
        # check the current screenshot for the harvestable tooltip using match template
        result = cv.matchTemplate(
            self.screenshot, self.harvest_tooltip, cv.TM_CCOEFF_NORMED
        )
        ...
        if max_val >= self.TOOLTIP_MATCH_THRESHOLD:
            ...
            offset = (
                mouse_position[0] - screen_loc[0],
                mouse_position[1] - screen_loc[1],
            )
    ...
    # TODO Probably not needed? We probably dont need to remember an exectued action as the bounding box around the object will disappear after we mined it
    ...
    def run(self):
        while not self.stopped:
            if self.state == BotState.INITIALIZING:
                ...
                # check the given bounding boxes, confirm a harvestable deposit (by class/category), then walk towards it until your in front (tooltip will appear if near enough)
